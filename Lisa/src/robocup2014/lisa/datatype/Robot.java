...
public class Robot
{
	...
	public static Direction toDirection(int arg0)
	{
	    ...
	    if      (arg0 == 0) toRtn = Direction.WEST;
	    else if (arg0 == 1) toRtn = Direction.NORTH;
	    else if (arg0 == 2) toRtn = Direction.EAST;
	...
	if(path.empty())
	{
	    ...
	    do
	    {
		//if the toVisit stack is empty, it meaning that we have explored all the maze
		if(!toVisit.isEmpty())
		{
		    end = toVisit.pop();
		}
	...
	if      (currentPoint.x == position.x + 1) absoluteDirToGo = Direction.EAST;
	else if (currentPoint.x == position.x - 1) absoluteDirToGo = Direction.WEST;
	else if (currentPoint.y == position.y + 1) absoluteDirToGo = Direction.NORTH;
	...
	setMaxPriority();
	// create a new comparator, based on the priority of the tiles and a new priorityqueue
