...
public class LisaActivity extends Activity
{
    ...
    // cosi facendo quando viene premuto il tasto back l'activity passa in
    // background
	...
	{
	case '1':
	    floor = Tile.Floor.SILVER_FLOOR;
	    break;
	case '2':
	    floor = Tile.Floor.BLACK_FLOOR;
	    break;
	default:
	    floor = Tile.Floor.WHITE_FLOOR;
	    break;
	}
	lisa.setLevel(level);
	...
	public boolean onTouch(View v, MotionEvent event)
	{
	    ...
	    if (!mazeView.getShowFirstFloor() || followRobot)
	    {
		return false;
	    }
		...
		else if (mode == ZOOM)
		{
		    ...
		    // Log.d("Range", scale +"::"+ endPoint[0] +"::"+
		    // endPoint[1] +"::"+ startPoint[0] +"::"+ startPoint[1]);
	    ...
	    case UP_1:
	    case UP_2:
		// endPoint[1] = new Point((int) event.getX(1), (int)
		// event.getY(1));
		mode = DRAG;
		toRtn = true;
		break;
	...
	public void setShowFirstFloor(boolean b)
	{
	    ...
	    // Log.d("Drawing",
	    // x_field_dim+":"+y_field_dim+":"+x_margin+":"+y_margin+":"
	...
	public void DrawField(Canvas canvas)
	{
	    ...
	    int rob_radius = cellDim / 2, rob_x = (lisa.getPosition().x * cellDim) + xMargin + rob_radius, rob_y = (lisa.getPosition().y * cellDim) + yMargin + rob_radius;
	    ...
	    // Log.d("Scaling", scale + ":" + mazeCenter.x + ":" +
	    // mazeCenter.y);
		...
		for (int y = 0; y < yFieldDim; y++)
		{
		    ...
		    {
			int start_x = (x * cellDim) + xMargin, start_y = (y * cellDim) + yMargin, stop_x = start_x, stop_y = start_y + cellDim;
